...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    ...
    const teacher = user; // Local constant to help TS with null checking in async function
    async function fetchData() {
      try {
        const [slotsRes, usersRes] = await Promise.all([
          fetch(`/api/slots?department=${teacher.department}`),
          fetch("/api/users"),
        ])

        const [slotsData, usersData] = await Promise.all([slotsRes.json(), usersRes.json()])

        const teacherSlots = slotsData.filter((s: Slot) => s.teacherId === teacher.id)
